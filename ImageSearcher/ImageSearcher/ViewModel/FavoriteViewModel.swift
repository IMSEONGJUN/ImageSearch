...
final class FavoriteViewModel: ViewModelType {
    ...
    private func fetchData(loadingCompleted: PublishRelay<Bool>) -> Driver<Result<[ImageInfo], FavoriteError>> {
        Observable.merge(
    ...
    private func dataSources(initialFetch: Driver<Result<[ImageInfo], FavoriteError>>) -> Driver<[ImageInfo]> {
        initialFetch
    ...
    private func errorMessage(initialFetch: Driver<Result<[ImageInfo], FavoriteError>>) -> Signal<String> {
        initialFetch
